...
#define PS_HEADER_TYPE_START struct { uint16_t length; char name[20];
#define PS_HEADER_TYPE_END(type, type_name) } ps_type_ ## type_name;


#define PS_HEADER_MEMBER(member) struct { uint16_t length; uint16_t offset; char name[20];  } ps_member_ ## member;
...
#define __PS_SRC_FUNC_ADD_TYPE(type, type_name) strncat(ps_global.ps_type_ ## type_name.name, #type_name, 20);  ps_global.ps_type_ ## type_name ## .length = sizeof(type);
#define __PS_SRC_FUNC_ADD_TYPE_MEMBER(type, type_name, member)  strncat(ps_global.ps_type_ ## type_name ## .ps_member_ ## member ## .name , #member, 20); \

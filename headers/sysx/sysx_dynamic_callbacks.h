...
#define DEFINE_SYSV_FUNCTON_END_DEF(name, rettype)					, uint64_t ___magic, void * ____disptr) { if (___magic != SYS_V_MAGIC) panicf("ILLEGAL SYSTEMV FUNCTION CALL - " #name " - CHECK FUNCTION DEFINTION AND PARAMETER COUNT"); thread_fpu_lock(); 
#define DEFINE_SYSV_FUNCTON_END										thread_fpu_unlock(); }

...
#define DEFINE_SYSV_FUNCTON_END_DEF(name, rettype)					uint64_t ___magic, void * ____disptr) { uint64_t __n; if (((size_t)(&__n) % 16) != 0) panicf("Xenus: not aligned (%s)", __FUNCTION__); bool __mgn_fpu = false; if (___magic != SYS_V_MAGIC) panicf("ILLEGAL SYSTEMV FUNCTION CALL - " #name " - CHECK FUNCTION DEFINTION AND PARAMETER COUNT"); __mgn_fpu  = thread_fpu_lock();
